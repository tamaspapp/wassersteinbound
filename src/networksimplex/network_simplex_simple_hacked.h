...
*
* Hacked by Tamas P. Papp (2024) to avoid copying the cost matrix --- TODO: code is currently borked. The cost matrix does not assign correctly.
* - The cost matrix is fed in as a row-major C array which MUST BE PADDED BY 
* AN EXTRA 2 * (rows + columns) ENTRIES at the end, which will store the costs 
* of the dummy source/sink nodes.
*  - This saves memory, but still have an O(rows * column) memory overhead due to generating and storing a dense bipartite graph.
*
...
namespace lemon {
	...
	public:
		...
		NetworkSimplexSimple(C *cost_matrix, const GR& graph, bool arc_mixing, int nbnodes, ArcsType nb_arcs, size_t maxiters = 0) :
   _cost(cost_matrix), // Careful: need to pad it by enough extra entries, otherwise WILL SEGFAULT. E.g. "double *cost_matrix = new double [nb_arcs + 2*nbnodes];"
   _graph(graph),
 ...
 _init_nb_nodes(nbnodes), _init_nb_arcs(nb_arcs)
		{
			...
			reset(); // Will actually just initialize some vectors.
			max_iter = maxiters;
		}
	...
	private:
		...
		private:
			...
			Cost* _cost; //const CostVector &_cost;
...
//         }
	...
	// 	}
		...
		NetworkSimplexSimple& reset() {
			...
			//_cost.resize(max_arc_num); // Will segfault unless the "_cost" array has at least "max_arc_num" = m * n + 2 * (m + n) entries for an m-by-n problem.
