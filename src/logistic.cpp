// Samplers for a multivariate logistic target
...
// Multivariate "logistic" target: pi(x) \propto e^norm_x / (1 + e^norm_x)^2
inline double logpi(const Eigen::Ref<const Eigen::VectorXd> &x)
{
    double y = x.norm();
    double exp_minus_y = exp(-y);
    return -(y + 2. * log1p(exp_minus_y));
}
inline Eigen::VectorXd gradlogpi(const Eigen::Ref<const Eigen::VectorXd> &x)
{   
    double y = x.norm();
    double exp_minus_y = exp(-y);
    return x * (-1. + 2. * exp_minus_y / (1. + exp_minus_y)) / y;
}
...
Rcpp::List RWMLogistic(const Eigen::Map<Eigen::VectorXd> &x0, 
                       const double &h, 
                       const int &iter)
{   
    // Constants
...
//' @export
//[[Rcpp::export(rng = false)]]
Rcpp::List MALALogistic(const Eigen::Map<Eigen::VectorXd> &x0, 
                        const double &h, 
                        const int &iter)
{
    // Constants
