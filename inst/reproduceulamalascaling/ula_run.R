...
for(k in 1:length(D)) {
  ...
  out <- foreach(i = 1:R, .packages = "wassersteinbound") %dopar% {
    SetSeed_cpp(seed, i)
    mcmc <- SimulateReflMaxULA_scaling(Sigma_inv, U, disp, h, L, iter, iter_final, thin)
...
for (i in 1:9) {
  ula[[i]]$w2_bound_components <- NULL
}

save(ula, file = "ula.Rdata")
